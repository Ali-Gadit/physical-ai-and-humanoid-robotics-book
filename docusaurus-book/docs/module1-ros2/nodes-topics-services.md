...
if __name__ == '__main__':
  ...
  <div className="urdu">
    ...
    ایک node ایک عمل (process) ہے جو کمپیوٹیشن کرتا ہے۔ ایک مکمل روبوٹ سسٹم بہت سے nodes پر مشتمل ہوتا ہے جو مل کر کام کرتے ہیں۔

    *   مثال: ایک node لیزر رینج فائنڈر کو کنٹرول کرتا ہے، ایک node وہیل موٹرز کو کنٹرول کرتا ہے، ایک node لوکلائزیشن (localization) کرتا ہے۔

    ## Topics

    Nodes ایک دوسرے کے ساتھ topics پر پیغامات شائع (publishing messages) کرکے بات چیت کرتے ہیں۔

    *   **Publisher**: ڈیٹا بھیجتا ہے۔
    *   **Subscriber**: ڈیٹا وصول کرتا ہے۔
    *   **Topic**: ایک نامی بس جس پر nodes پیغامات کا تبادلہ کرتے ہیں۔

    ## Services

    Services نوڈز کے بات چیت کرنے کا ایک اور طریقہ ہیں۔ Topics (یک طرفہ) کے برعکس، services **کال اور رسپانس (Call-and-Response)** ماڈل پر مبنی ہیں۔

    *   **Client**: درخواست (request) بھیجتا ہے۔
    *   **Server**: جواب (response) بھیجتا ہے۔
  </div>
</BilingualChapter>
