...
<BilingualChapter>
  ...
  <div className="urdu">
    # Python اور ROS 2 انٹیگریشن

    ## ایک سادہ Publisher لکھنا

    یہاں ایک بنیادی Python اسکرپٹ ہے جو "Hello World" کو ایک topic پر شائع (publish) کرتا ہے۔
    ...
    1.  **Import rclpy**: Python کے لیے ROS 2 کلائنٹ لائبریری۔
    2.  **Inherit from Node**: ایک کلاس بنائیں جو آپ کے ROS node کی نمائندگی کرے۔
    3.  **create_publisher**: ٹاپک کا نام اور میسج کی قسم سیٹ کرتا ہے۔
    4.  **create_timer**: وقتاً فوقتاً ایک فنکشن کو کال کرتا ہے۔
  </div>
</BilingualChapter>
