...
<BilingualChapter>
  ...
  <div className="urdu">
    # طبیعیات کی سیمولیشن (Physics Simulation)

    ## Rigid Body Dynamics

    Gazebo لنکس کو سخت اجسام (rigid bodies) سمجھ کر طبیعیات کی نقالی کرتا ہے۔ آپ کو وضاحت کرنی ہوگی:
    *   **Mass (کمیت)**: کلوگرام (kg) میں۔
    *   **Inertia (جمود)**: گردش کے خلاف مزاحمت۔
    *   **Friction (رگڑ)**: سطح کے تعامل کی خصوصیات۔
    ...
    جب دو اشیاء آپس میں ٹچ کرتی ہیں تو ایک contact force پیدا ہوتی ہے۔ غلط تصادم میشز (collision meshes) روبوٹ کے دھماکے کا سبب بن سکتے ہیں (یعنی سیمولیشن میں بکھر سکتے ہیں)۔
  </div>
</BilingualChapter>
