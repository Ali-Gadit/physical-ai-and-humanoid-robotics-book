...
<BilingualChapter>
  ...
  <div className="urdu">
    # Unity انٹیگریشن (اختیاری)

    ## Unity کیوں؟

    اگرچہ Gazebo ROS کے لیے معیار ہے، Unity مخصوص کاموں (جیسے reinforcement learning) کے لیے اعلی گرافکس اور طبیعیات کا استحکام (physics stability) پیش کرتا ہے۔

    ## ROS-TCP کنیکٹر

    Unity `ROS-TCP-Connector` پیکیج کے ذریعے ROS 2 کے ساتھ بات چیت کرتا ہے۔

    1.  پیکیج کو Unity میں درآمد (Import) کریں۔
    2.  اپنی ROS مشین کا IP ایڈریس سیٹ کریں۔
    3.  Unity Editor میں براہ راست topics کا تصور (Visualize) کریں۔
  </div>
</BilingualChapter>
