...
<BilingualChapter>
  ...
  <div className="urdu">
    # وائس ٹو ایکشن: صوتی احکامات کے لیے OpenAI Whisper کا استعمال

    ## تعارف

    وائس ٹو ایکشن سسٹمز صارفین کو بولے گئے احکامات کے ذریعے روبوٹس کو کنٹرول کرنے کی اجازت دے کر قدرتی انسانی روبوٹ تعامل کو ممکن بناتے ہیں۔ OpenAI Whisper، جو کہ ایک جدید ترین خودکار اسپیچ ریکگنیشن (ASR) سسٹم ہے، ہیومنائیڈ روبوٹس میں مضبوط وائس کمانڈ کی شناخت کے لیے بنیاد فراہم کرتا ہے۔ یہ ٹیکنالوجی روبوٹ کو قدرتی زبان کو سمجھنے اور جواب دینے کے قابل بناتی ہے، جس سے وہ زیادہ قابل رسائی اور بدیہی بن جاتے ہیں۔

    انسانی مرکوز ماحول میں کام کرنے والے ہیومنائیڈ روبوٹس کے لیے، صوتی انٹرفیس خاص طور پر قیمتی ہیں کیونکہ وہ ہینڈز فری تعامل اور قدرتی مواصلات کے نمونوں کو فعال کرتے ہیں جن کے انسان عادی ہیں۔

    ## OpenAI Whisper کو سمجھنا

    ### آرکیٹیکچر اور صلاحیتیں

    OpenAI Whisper ایک ٹرانسفارمر پر مبنی نیورل نیٹ ورک ہے جو کثیر لسانی اور ملٹی ٹاسک زیر نگرانی ڈیٹا کے ایک بڑے ڈیٹا سیٹ پر تربیت یافتہ ہے۔ اس کی اہم صلاحیتوں میں شامل ہیں:

    *   **کثیر لسانی سپورٹ**: 99+ زبانوں میں شناخت۔
    *   **مضبوط کارکردگی**: شور والے ماحول میں اچھا کام کرتا ہے۔
    *   **اسپیکر کی شناخت**: مختلف بولنے والوں کے درمیان تمیز کر سکتا ہے۔
    *   **ٹائم اسٹیمپنگ**: تقریر کے حصوں کے لیے وقت کی معلومات فراہم کرتا ہے۔
    *   **اوقاف اور کیپٹلائزیشن**: مناسب طریقے سے فارمیٹ شدہ متن آؤٹ پٹ کرتا ہے۔

    ### Whisper ماڈلز

    Whisper مختلف کارکردگی کی خصوصیات کے ساتھ کئی سائز میں آتا ہے:

    *   **tiny**: ~39M پیرامیٹرز، سب سے تیز لیکن کم سے کم درست۔
    *   **base**: ~74M پیرامیٹرز، رفتار اور درستگی کا اچھا توازن۔
    *   **small**: ~244M پیرامیٹرز، بہتر درستگی۔
    *   **medium**: ~769M پیرامیٹرز، اعلی درستگی۔
    *   **large**: ~1550M پیرامیٹرز، سب سے زیادہ درستگی، سب سے سست۔

    ہیومنائیڈ روبوٹس کے لیے، انتخاب کا انحصار ہارڈ ویئر کی صلاحیتوں اور ریئل ٹائم ضروریات پر ہوتا ہے۔

    ## روبوٹکس کے لیے Whisper کی انسٹالیشن

    ### پیشگی شرائط

    ```bash
    # مطلوبہ انحصار (dependencies) انسٹال کریں
    pip install torch torchvision torchaudio
    pip install openai-whisper
    pip install pyaudio  # آڈیو ان پٹ کے لیے
    pip install sounddevice  # متبادل آڈیو لائبریری
    pip install numpy
    pip install transformers  # اگر اضافی NLP پروسیسنگ استعمال کر رہے ہیں
    ```

    ### ہارڈویئر کے تقاضے

    *   **مائیکروفون**: آواز کیپچر کے لیے USB مائیکروفون یا سرنی۔
    *   **پروسیسنگ پاور**: ریئل ٹائم پروسیسنگ کے لیے GPU کی سفارش کی جاتی ہے۔
    *   **میموری**: بیس ماڈل کے لیے 2GB+ RAM، بڑے ماڈل کے لیے 8GB+۔
    ...
    class WhisperVoiceToAction:
        def __init__(self, model_size="base", device="cuda" if torch.cuda.is_available() else "cpu"):
            """
            Whisper voice-to-action سسٹم کو شروع کریں

            Args:
                model_size: Whisper ماڈل کا سائز (tiny, base, small, medium, large)
                device: ماڈل چلانے کے لیے ڈیوائس (cuda, cpu)
            """
            print(f"Loading Whisper {model_size} model on {device}...")
            self.model = whisper.load_model(model_size).to(device)
            self.device = device
            ...
            # آڈیو کنفیگریشن
            self.rate = 16000  # نمونہ کی شرح
            self.chunk = 1024  # بفر کا سائز
            self.format = pyaudio.paInt16
            self.channels = 1
        ...
        def record_audio(self, duration=5):
            ...
            Args:
                duration: ریکارڈنگ کا دورانیہ سیکنڈوں میں

            Returns:
                آڈیو ڈیٹا بطور numpy array
            """
            print(f"Recording for {duration} seconds...")
        ...
        def transcribe_audio(self, audio_data):
            """
            Whisper کا استعمال کرتے ہوئے آڈیو کو ٹرانسکرائب کریں

            Args:
                audio_data: آڈیو ڈیٹا بطور numpy array

            Returns:
                ٹرانسکرائب شدہ متن
            """
            # ٹینسر میں تبدیل کریں
            audio_tensor = torch.from_numpy(audio_data).to(self.device)
        ...
        def continuous_listening(self, callback_func=None):
            """
            مسلسل صوتی احکامات سنیں

            Args:
                callback_func: ٹرانسکرائب شدہ متن کے ساتھ کال کرنے کا فنکشن
            """
            print("Starting continuous listening...")
            print("Say 'quit' to exit")
            ...
            while True:
                try:
                    # آڈیو ریکارڈ کریں
                    audio_data = self.record_audio(duration=3)  # 3 سیکنڈ کی ریکارڈنگ
    ...
    # استعمال کی مثال
    def command_callback(text):
        """تسلیم شدہ احکامات پر کارروائی کرنے کے لیے کال بیک فنکشن"""
        print(f"Processing command: {text}")

        # یہاں آپ کمانڈ پروسیسنگ منطق کو نافذ کریں گے
        # مثال کے طور پر، روبوٹ ایکشنز کے لیے کمانڈز کا نقشہ بنانا
        if "move" in text.lower():
            print("Robot should move!")
        elif "stop" in text.lower():
            print("Robot should stop!")
    ...
    def main():
        # وائس ٹو ایکشن سسٹم کو شروع کریں
        vta = WhisperVoiceToAction(model_size="base")

        try:
            # مسلسل سننا شروع کریں
            vta.continuous_listening(callback_func=command_callback)
        finally:
            vta.close()
    ...
    ## جدید وائس کمانڈ پروسیسنگ

    ### کمانڈ کی شناخت اور تجزیہ
    ...
    class VoiceCommandProcessor:
        def __init__(self):
            """پہلے سے طے شدہ کمانڈ پیٹرن کے ساتھ کمانڈ پروسیسر کو شروع کریں"""
            self.command_patterns = {
                # نیویگیشن کمانڈز
                "move_forward": [r"move forward", r"go forward", r"move ahead", r"go ahead"],
                "move_backward": [r"move backward", r"go backward", r"move back", r"go back"],
                ...
                # پیرامیٹرز کے ساتھ پیچیدہ کمانڈز
                "navigate_to": [r"go to (.+)", r"move to (.+)", r"navigate to (.+)"],
                "find_object": [r"find (.+)", r"locate (.+)", r"search for (.+)"]
            }

            # آبجیکٹ کی شناخت کے نمونے
            self.object_patterns = [
                r"(\w+) (table|chair|cup|bottle|box|door|window)",
                r"(red|blue|green|yellow|big|small) (\w+)",
                r"(\w+) (left|right|front|back)"
        ...
        def extract_command(self, text: str) -> Dict:
            """
            متن سے کمانڈ اور پیرامیٹرز نکالیں

            Args:
                text: تسلیم شدہ تقریر کا متن

            Returns:
                کمانڈ کی قسم اور پیرامیٹرز کے ساتھ لغت
            """
            text_lower = text.lower().strip()
            ...
            for command_type, patterns in self.command_patterns.items():
                for pattern in patterns:
                    match = re.search(pattern, text_lower)
                    if match:
                        # پیرامیٹرز نکالیں اگر موجود ہوں
                        params = match.groups() if match.groups() else []
                        ...
                        return {
                            ...
                            "original_text": text,
                            "parameters": list(params),
                            "confidence": 0.9  # پلیس ہولڈر اعتماد
                        }

            # اگر کوئی مخصوص کمانڈ نہیں ملا تو عام کمانڈ کے طور پر واپس کریں
            return {
                "command": "unknown",
                "original_text": text,
                "parameters": [],
        ...
        def process_command(self, text: str) -> Optional[Dict]:
            """
            وائس کمانڈ پر کارروائی کریں اور قابل عمل ایکشن واپس کریں

            Args:
                text: تسلیم شدہ تقریر کا متن

            Returns:
                ایکشن لغت یا None اگر کمانڈ تسلیم نہیں کیا گیا
            """
            command_info = self.extract_command(text)
            ...
            # کمانڈ کو روبوٹ ایکشن میں تبدیل کریں
            action = self.command_to_action(command_info)
            return action

        def command_to_action(self, command_info: Dict) -> Dict:
            """
            کمانڈ کی معلومات کو روبوٹ ایکشن میں تبدیل کریں

            Args:
                command_info: extract_command سے کمانڈ کی معلومات
    ...
    # Whisper سسٹم کے ساتھ انٹیگریشن
    class AdvancedWhisperVoiceToAction(WhisperVoiceToAction):
        def __init__(self, model_size="base", device="cuda" if torch.cuda.is_available() else "cpu"):
            super().__init__(model_size, device)
            self.command_processor = VoiceCommandProcessor()
        ...
        def process_voice_command(self, text: str) -> Optional[Dict]:
            ...
            Returns:
                روبوٹ ایکشن لغت یا None
            """
            action = self.command_processor.process_command(text)

            if action:
                # کمانڈ ہسٹری میں شامل کریں
                self.command_history.append({
                    "timestamp": time.time(),
                    "original_text": text,
                    "action": action
        ...
        def continuous_listening_with_processing(self):
            """مسلسل سنیں اور کمانڈز پر کارروائی کریں"""
            print("Starting continuous listening with command processing...")
            print("Say 'quit' to exit")
            ...
            while True:
                try:
                    ...
                    if text:
                        ...
                        if action:
                            # ایکشن پر عمل کریں (حقیقی نفاذ میں، یہ روبوٹ کو بھیجے گا)
                            self.execute_action(action)
        ...
        def execute_action(self, action: Dict):
            """
            روبوٹ ایکشن پر عمل کریں

            Args:
                action: کمانڈ پروسیسنگ سے ایکشن لغت
            """
            print(f"Executing action: {action}")
            # حقیقی نفاذ میں، یہ روبوٹ کو کمانڈ بھیجے گا
            # ROS 2 یا دوسرے مواصلاتی انٹرفیس کے ذریعے

            # مثال کے طور پر ایکشن ایگزیکیوشن
            if action["type"] == "navigation":
                if action["action"] == "move_straight":
                    distance = action["parameters"]["distance"]
                    print(f"Moving straight {distance} meters")
    ...
    def main():
        # ایڈوانسڈ وائس ٹو ایکشن سسٹم کو شروع کریں
        vta = AdvancedWhisperVoiceToAction(model_size="base")

        try:
            # پروسیسنگ کے ساتھ مسلسل سننا شروع کریں
            vta.continuous_listening_with_processing()
        finally:
            vta.close()
    ...
    class VoiceCommandNode(Node):
        def __init__(self):
            ...
            # آڈیو ان پٹ کے لیے سبسکرائبرز (اگر بیرونی آڈیو ماخذ استعمال کر رہے ہیں)
            self.audio_sub = self.create_subscription(
                AudioData,
                '/audio_input',
                self.audio_callback,
        ...
        def record_audio_thread(self):
            ...
            while self.recording:
                ...
                if len(frames) * self.chunk >= self.rate * 3:  # 3 سیکنڈ کی آڈیو
                    # numpy array میں تبدیل کریں اور قطار میں شامل کریں
                    audio_data = b''.join(frames)
                    audio_np = np.frombuffer(audio_data, dtype=np.int16)
                    audio_np = audio_np.astype(np.float32) / 32768.0
        ...
        def audio_callback(self, msg):
            """بیرونی ماخذ سے آڈیو ڈیٹا کے لیے کال بیک"""
            # آڈیو ڈیٹا کو numpy array میں تبدیل کریں
            audio_np = np.frombuffer(msg.data, dtype=np.int16)
            audio_np = audio_np.astype(np.float32) / 32768.0
        ...
        def process_audio(self):
            """قطار سے آڈیو پر عمل کریں"""
            try:
                while True:  # قطار میں موجود تمام آڈیو پر عمل کریں
                    audio_data = self.audio_queue.get_nowait()
                ...
                pass  # مزید آڈیو پروسیس کرنے کے لیے نہیں ہے
        ...
        def execute_action(self, action):
            """کمانڈ کی بنیاد پر روبوٹ ایکشن پر عمل کریں"""
            self.get_logger().info(f"Executing action: {action}")
        ...
        def move_straight(self, distance):
            """مخصوص فاصلے کے لیے روبوٹ کو سیدھا منتقل کریں"""
            cmd = Twist()
            cmd.linear.x = 0.5 if distance > 0 else -0.5  # رفتار کو ضرورت کے مطابق ایڈجسٹ کریں
            cmd.angular.z = 0.0

            # فاصلے کی بنیاد پر مدت کے لیے کمانڈ شائع کریں
            duration = abs(distance) / 0.5  # فرض کریں 0.5 m/s رفتار
            self.publish_for_duration(cmd, duration)

        def rotate(self, angle):
            """مخصوص زاویہ سے روبوٹ کو گھمائیں"""
            cmd = Twist()
            cmd.linear.x = 0.0
            cmd.angular.z = 0.5 if angle > 0 else -0.5  # کونیی رفتار کو ضرورت کے مطابق ایڈجسٹ کریں

            # زاویہ کو وقت میں تبدیل کریں (مستقل کونیی رفتار فرض کرتے ہوئے)
            duration = abs(angle) / 180.0 * 3.14  # آسان حساب کتاب
            self.publish_for_duration(cmd, duration)

        def stop_robot(self):
            """روبوٹ کی نقل و حرکت بند کریں"""
            cmd = Twist()
            cmd.linear.x = 0.0
            cmd.angular.z = 0.0
            self.cmd_vel_pub.publish(cmd)
        ...
        def speak(self, text):
            """روبوٹ سے متن بلوائیں"""
            msg = String()
            msg.data = text
            self.speech_pub.publish(msg)

        def publish_for_duration(self, cmd, duration):
            """مخصوص مدت کے لیے کمانڈ شائع کریں"""
            start_time = self.get_clock().now()
            ...
            # مناسب طریقے سے اختتامی وقت کا حساب لگائیں
            end_time = start_time + Duration(seconds=duration_msg.sec, nanoseconds=duration_msg.nanosec)

            while self.get_clock().now() < end_time:
                self.cmd_vel_pub.publish(cmd)
                # rclpy.spin_once کے بجائے مناسب شرح استعمال کریں
                time.sleep(0.1)  # 100ms کے لیے سو جائیں
    ...
    ## جدید وائس پروسیسنگ خصوصیات

    ### وائس ایکٹیویٹی ڈیٹیکشن
    ...
    class VoiceActivityDetector:
        def __init__(self, sample_rate=16000, frame_duration_ms=30, mode=3):
            """
            وائس ایکٹیویٹی ڈیٹیکٹر کو شروع کریں

            Args:
                sample_rate: آڈیو نمونہ کی شرح (8000, 16000, 32000, or 48000)
                frame_duration_ms: ملی سیکنڈ میں فریم کا دورانیہ (10, 20, or 30)
                mode: جارحیت موڈ (0-3، زیادہ کا مطلب زیادہ جارحانہ ہے)
            """
            self.vad = webrtcvad.Vad(mode)
            self.sample_rate = sample_rate
            self.frame_duration_ms = frame_duration_ms
            self.frame_size = int(sample_rate * frame_duration_ms / 1000) * 2  # 2 بائٹس فی نمونہ

            # وائس ایکٹیویٹی کا پتہ لگانے کے لیے بفر
            self.ring_buffer = collections.deque(maxlen=30)  # 30 فریم = 900ms
            self.triggered = False
            self.vad_frames = []

        def is_speech(self, audio_data):
            """
            پتہ لگائیں کہ کیا آڈیو میں تقریر ہے

            Args:
                audio_data: آڈیو ڈیٹا بطور بائٹس

            Returns:
                بولین جو ظاہر کرتا ہے کہ آیا تقریر کا پتہ چلا ہے
            """
            # فریمز میں تبدیل کریں
            frames = self._bytes_to_frames(audio_data)
            ...
            # تقریر کے طور پر غور کریں اگر 20% سے زیادہ فریموں میں تقریر ہو
            speech_ratio = speech_frames / total_frames if total_frames > 0 else 0
            return speech_ratio > 0.2

        def _bytes_to_frames(self, audio_data):
            """VAD کے لیے آڈیو بائٹس کو فریموں میں تبدیل کریں"""
            frames = []
            for i in range(0, len(audio_data), self.frame_size):
                frame = audio_data[i:i + self.frame_size]
                if len(frame) == self.frame_size:
    ...
    # VAD کے ساتھ بہتر وائس ٹو ایکشن
    class VADEnhancedVoiceToAction(AdvancedWhisperVoiceToAction):
        def __init__(self, model_size="base", device="cuda" if torch.cuda.is_available() else "cpu"):
            super().__init__(model_size, device)
            self.vad_detector = VoiceActivityDetector()

            # وائس ایکٹیویٹی کی حالت
            self.listening_for_speech = False
            self.speech_buffer = b""
            self.max_silence_frames = 10  # رکنے سے پہلے زیادہ سے زیادہ خاموشی کے فریم
            self.silence_counter = 0

        def record_with_vad(self, max_duration=10):
            """
            وائس ایکٹیویٹی ڈیٹیکشن کے ساتھ آڈیو ریکارڈ کریں

            Args:
                max_duration: زیادہ سے زیادہ ریکارڈنگ کا دورانیہ سیکنڈوں میں

            Returns:
                تقریر پر مشتمل آڈیو ڈیٹا
            """
            stream = self.audio.open(
                format=self.format,
                channels=self.channels,
            ...
            while frame_count < max_frames:
                ...
                # وائس ایکٹیویٹی چیک کریں
                if self.vad_detector.is_speech(data):
                    silence_frames = 0  # خاموشی کاؤنٹر ری سیٹ کریں
                    print(".", end="", flush=True)  # بصری تاثرات
                else:
                    silence_frames += 1
                    print(" ", end="", flush=True)  # بصری تاثرات

                # اگر ہمارے پاس کافی خاموشی ہے، تو ریکارڈنگ بند کریں
                if silence_frames > 15 and len(frames) > 10:  # کم از کم 10 فریم ریکارڈ کیے گئے
                    print(f"\nSilence detected, stopping after {silence_frames} silent frames")
                    break
    ...
    ### ریئل ٹائم پروسیسنگ کے لیے ماڈل کی اصلاح

    ```python
    class OptimizedWhisperProcessor:
        def __init__(self, model_size="base"):
            """آپٹمائزڈ Whisper پروسیسر کو شروع کریں"""
            # اصلاحات کے ساتھ ماڈل لوڈ کریں
            self.model = whisper.load_model(model_size, device="cuda" if torch.cuda.is_available() else "cpu")
            ...
            # بہتر کارکردگی کے لیے ٹینسرز پہلے سے مختص کریں
            self.max_audio_length = 32000 * 30  # 30 سیکنڈ 16kHz پر
            self.audio_buffer = torch.zeros(self.max_audio_length, dtype=torch.float32)

        def transcribe_optimized(self, audio_data):
            """پہلے سے مختص ٹینسرز کے ساتھ آپٹمائزڈ ٹرانسکرپشن"""
            # ٹینسر میں تبدیل کریں
            audio_tensor = torch.from_numpy(audio_data).to(self.model.device)

            # مستقل کارکردگی کے لیے پیڈ یا معیاری لمبائی تک ٹرم کریں
            if len(audio_tensor) < self.max_audio_length:
                padded = torch.zeros(self.max_audio_length, dtype=audio_tensor.dtype, device=audio_tensor.device)
                padded[:len(audio_tensor)] = audio_tensor
            else:
    ...
    ## وائس ٹو ایکشن کے لیے بہترین طریقے

    ### 1. غلطی کو سنبھالنا اور مضبوطی (Error Handling and Robustness)
    ...
    class RobustVoiceToAction:
        ...
        def robust_transcribe(self, audio_data):
            """غلطی کو سنبھالنے اور دوبارہ کوشش کرنے کے ساتھ ٹرانسکرائب کریں"""
            for attempt in range(self.retry_count):
                try:
                    text = self.transcribe_audio(audio_data)

                    # چیک کریں کہ آیا ٹرانسکرپشن معقول ہے
                    if self.is_reasonable_transcription(text):
                        return text
                    else:
                        print(f"Attempt {attempt + 1}: Transcription seems unreasonable: '{text}'")
                ...
                if attempt < self.retry_count - 1:
                    time.sleep(0.5)  # دوبارہ کوشش کرنے سے پہلے مختصر وقفہ

            return None  # تمام کوششوں کے بعد ناکام
        ...
        def is_reasonable_transcription(self, text):
            ...
            # عام Whisper غلطیوں کی جانچ کریں
            if text.strip().lower() in ["", "thank you", "thanks", "bye", "goodbye"]:
                return False

            # ضرورت سے زیادہ اوقاف یا خصوصی حروف کی جانچ کریں
            special_chars = sum(1 for c in text if c in "!@#$%^&*()_+-=[]{}|;:,.<>?")
            if special_chars > len(text) * 0.3:  # 30% سے زیادہ خصوصی حروف
                return False

            return True
    ```

    ### 2. سیاق و سباق سے آگاہ پروسیسنگ (Context-Aware Processing)
    ...
    class ContextAwareVoiceProcessor:
        ...
        def process_command_with_context(self, text):
            """موجودہ سیاق و سباق پر غور کرتے ہوئے کمانڈ پر کارروائی کریں"""
            # کمانڈ کی بنیاد پر سیاق و سباق کو اپ ڈیٹ کریں
            if "move" in text.lower() or "go" in text.lower():
                self.context["robot_state"] = "navigating"
            elif "stop" in text.lower():
                self.context["robot_state"] = "stopped"
            ...
            # اگر ضرورت ہو تو ایکشن میں سیاق و سباق شامل کریں
            if action:
                action["context"] = self.context.copy()
    ...
    ### آڈیو کوالٹی کے مسائل
    *   **پس منظر کا شور**: شور کو دبانے یا بہتر مائیکروفون استعمال کریں۔
    *   **کلپنگ**: مائیکروفون گین کو کم کریں یا خودکار گین کنٹرول استعمال کریں۔
    *   **کم والیوم**: مائیکروفون کی حساسیت میں اضافہ کریں یا دشاتمک مائیکروفون استعمال کریں۔

    ### شناخت کی درستگی
    *   **ماڈل کا انتخاب**: اپنی ضروریات کے لیے مناسب ماڈل سائز کا انتخاب کریں۔
    *   **آڈیو پری پروسیسنگ**: شور میں کمی اور نارملائزیشن کا اطلاق کریں۔
    *   **زبان کی ترتیبات**: Whisper کے لیے صحیح زبان کی وضاحت کریں۔

    ### کارکردگی کے مسائل
    *   **تاخیر**: ماڈل لوڈنگ کو بہتر بنائیں اور اگر ضرورت ہو تو چھوٹے ماڈل استعمال کریں۔
    *   **میموری کا استعمال**: بڑے ماڈلز کے لیے CPU آف لوڈنگ کا استعمال کریں۔
    *   **ریئل ٹائم رکاوٹیں**: مسلسل ان پٹ کے لیے اسٹریمنگ ریکگنیشن کو نافذ کریں۔

    ## ہینڈس آن مشق

    ایک مکمل وائس ٹو ایکشن سسٹم بنائیں جس میں شامل ہوں:

    1.  مناسب ماڈل کے انتخاب کے ساتھ Whisper پر مبنی اسپیچ ریکگنیشن۔
    2.  غیر ضروری پروسیسنگ کو کم کرنے کے لیے وائس ایکٹیویٹی ڈیٹیکشن۔
    3.  روبوٹ ایکشنز کے لیے کمانڈ پارسنگ اور میپنگ۔
    4.  سیمولیٹڈ روبوٹ کو کنٹرول کرنے کے لیے ROS 2 انٹیگریشن۔
    5.  کمانڈ کی تفہیم کو بہتر بنانے کے لیے سیاق و سباق سے آگاہ پروسیسنگ۔
    6.  غلطی کو سنبھالنے اور مضبوطی کی خصوصیات۔

    یہ مشق آپ کو ہیومنائیڈ روبوٹس کے لیے صوتی انٹرفیس کو نافذ کرنے اور قدرتی انسانی روبوٹ تعامل میں شامل چیلنجوں کو سمجھنے کے ساتھ عملی تجربہ فراہم کرے گی۔
  </div>
</BilingualChapter>